#데이터 전처리
#원핫인코딩 각자리수에 해당하는 것을 1로 바꿔줌

#코랩에서 제공하는 구글에서 제공하는 온라인 파이선 개발환경
...
#원핫인코딩 함수를 만들어서 원핫인코딩하고 다시 원핫인코딩을 원래번호대로 변환하는 함수를 만듬
#원핫인코딩벡터
...
###5. 모델구성및 학습 LSTM (RNN계열 ) 시계열 데이터, 자연어 데이터처리등에 사용되는 딥러닝모델
#모델입출력저으이 -지도학습
